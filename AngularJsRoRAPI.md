...
1.3 Preparing ~/app/controllers/rails_objects_controller.rb
--

For this tutorial, we will only using the Index action but without the code generated by the scaffold.  We inted to make the calls to the database using the controller within the API module from which AngularJs will be fetching the data.
...
The code generated by the scaffold is of no use for this particular tutorial.  We will be displaying the data fetched by AngularJs rather than rendering an [Active-Record-object].
